		...
		Pass
		{
			...
			*/
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 view_direction = normalize(mul((float3x3)unity_ObjectToWorld, i.vertex));  //normalize(i.view_ray);
				...
				float fragment_angular_size = length(ddx(view_direction) + ddy(view_direction)) / length(view_direction);
